...
def anti_relative_neighborhodd_graph(data_size, data_embedding, ptnn, t, ptnd, bias_mid, stability_factors):
    """
    ...
    for i in range(data_size):
        if stability_factors[i] <= 0.2:
            angle_refine.append(0)
            continue
...
def tnn_2T(nums):
    Tmax_iter1 = np.mean(nums) + np.std(nums, ddof=0)
    ...
    Tmax_iter2 = np.mean(nums) + np.std(nums, ddof=0)
...
def Two_stage_thresholding(data_size, ptnn, alpha, t, cluster_label, rnns):
    fbp = []
    """  初始化 反向bp上离群，初始化值为－1，这里的－1不代表下离群－－－－－－－－－"""
    rbp = np.zeros((data_size, t)) - 1
    """ 计算xi的上离群性和tnn成员的上离群性， 并更新近邻的反向上离群性－－－－－－－－"""

    for i in range(data_size):
        ti_alphas = [alpha[i]]
        ti_alphas.extend([alpha[x].tolist() for x in ptnn[i]])
        ffbp = tnn_2T(ti_alphas)
        fbp.append(tnn_2T(ti_alphas))  # 保存邻居点的反向上离群，inver_bp
        for j in range(t):
            if rnns[i][j] == 1:
...
def pauta_criterion_alpha(data_size, alpha, ptnn, t, fbp, rbp, threshold_stability, shared_nn):
    ...
    for i in range(data_size):
        ...
        for j in range(t):
            xj = ptnn[i][j]
            alpha_j = alpha[xj]
            if abs(alpha_j - mean_t[i]) <= 2 * variance_t[i]:
                fpctemp.append(1)
            else:
                fpctemp.append(0)
            ...
            """# 计算xi近邻海拔的 拉依达法则的反向值"""
            if abs(alpha_i - mean_t[xj]) <= 2 * variance_t[xj]:
                rpctemp.append(1)
            else:
                rpctemp.append(0)
    ...
    if i in pknn[j]:
        ...
        for m in pknn[j][:hid+1]:
            if center[m] == center[i]:
                ...
                if abs(alpha[i] - mean_t[m]) <= 3 * variance_t[m] and abs(alpha[m] - mean_t[i]) <= 3 * variance_t[i] and flag_sknn:
                    same += 1
                else:
                    diff += 1
        dbv = same - diff
    ...
    for i in points:
        ...
        for m in range(t):
            ...
            if stability_factors[j] > threshold_stability:
                if len(set(pknn[i]).intersection(pknn[j])) >= int(k/2) and abs(alpha[i] - mean_t[j]) <= 3 * variance_t[j] and abs(alpha[j] - mean_t[i]) <= 3 * variance_t[i]:
                    """记录点i的可用knn"""
...
def assignment_remaining_points(points, center, pknn, mean_t, variance_t, alpha, k, boundary_flag):
    ...
    for j in points:
        if center[j] < 0:
            for i in pknn[j]:
                if len(set(pknn[i]).intersection(pknn[j])) >= int(k/2) and abs(alpha[i] - mean_t[j]) <= 3 * variance_t[j] and abs(alpha[j] - mean_t[i]) <= 3 * variance_t[i] and center[i] >= 0:
                    center[j] = center[i]
...
def wc_ca_clustering(data_embedding, t, k, threshold_stability):
    ...
    """---------------------------------------------------------------------------------"""
    fbp, rbp = Two_stage_thresholding(data_size, ptnn, alpha, t, cluster_label, rnns)
    ...
    diver_tnn_fine, angle, angle_refine, coo = anti_relative_neighborhodd_graph(data_size, data_embedding, ptnn, t, ptnd, bias_mid, stability_factors)
