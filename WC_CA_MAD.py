...
def large_nn(data_embedding, large_k):
    data_size = data_embedding.shape[0]
...
def tnn_mad(nums):
    m = np.median(nums)
    mad_value = 1.4826 * np.median(abs(np.array(nums) - m))
    Tmax = m + 3 * mad_value
...
def MAD_Median_Absolute_Deviation(data_size, ptnn, alpha, t, cluster_label, rnns):
    fbp = []
    ...
    for i in range(data_size):
        ...
        ffbp = tnn_mad(ti_alphas)
        fbp.append(tnn_mad(ti_alphas))  # 保存邻居点的反向上离群，inver_bp
        for j in range(t):
            if rnns[i][j] == 1:
...
def wc_ca_clustering(data_embedding, t, k, threshold_stability):
    ...
    large_k = k
    large_knn, large_knd = large_nn(data_embedding, large_k)
    ...
    """---------------------------------------------------------------------------------"""
    fbp, rbp = MAD_Median_Absolute_Deviation(data_size, ptnn, alpha, t, cluster_label, rnns)
